...
class MyHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<AttractionModel>>(
        future: fetchAttractions(http.Client()),
        builder: (context, snapshot) {
          if (snapshot.hasError) print (snapshot.error);

          return snapshot.hasData ?
              AttractionsList(attractions: snapshot.data)
...
class AttractionsList extends StatelessWidget {
  ...
  AttractionsList({Key key, this.attractions}) : super(key: key);
